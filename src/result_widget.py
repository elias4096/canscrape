...
class ResultWidget(QWidget):
    def __init__(self, settings_model: Settings):
        ...
        headers = ["ID","D1","D2","D3","D4","D5","D6","D7","D8","Byte Sum","Byte Entropy","Iso Score","Likelihood"]
    ...
    def on_isolation_forest_clicked(self):
        ...
        for i, row in enumerate(rows):
            ...
            self.table_left.setItem(i, 9, QTableWidgetItem(f"{row[9]:.1f}"))
            self.table_left.setItem(i, 10, QTableWidgetItem(f"{row[10]:.1f}"))
            self.table_left.setItem(i, 11, QTableWidgetItem(f"{row[11]:.4f}"))
            self.table_left.setItem(i, 12, QTableWidgetItem(f"{row[12]:.4f}"))
